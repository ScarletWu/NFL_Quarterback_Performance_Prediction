...
This paper presents an analysis of NFL quarterback performance over the recent seasons. Utilizing the `nflverse` dataset, we aim to explore aggregate performance metrics, identify trends, and model predictive performance factors. The absence of direct experience data challenges us to leverage alternative analytical strategies to understand quarterback effectiveness and predict future performance.

# Data Preparation

To begin our analysis, we filter the data for quarterbacks, focusing on the regular season performance of the last three years.
...
We aggregate key performance metrics for each quarterback across seasons to establish a foundation for in-depth analysis and comparison.
...
Visualizing these metrics in @fig-1 allows us to quickly identify top performers and observe trends over time. For instance, we explore the average passing yards per game for quarterbacks in the latest season.
...
# Predictive Modeling
To predict future quarterback performance, we develop a model based on the aggregated performance data. Our model aims to predict the average yards per game, a key indicator of quarterback effectiveness.
